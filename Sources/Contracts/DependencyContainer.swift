...
final class ServiceContainer {
    ...
    /// In-app auto-update (Sparkle-backed).
    ///
    /// Injected as a plain `any` existential (like `gitService`): views only
    /// trigger checks / read version, no `@Observable` property tracking needed.
